...
const Splash = () => {
  ...
  return (
    <div className="min-h-screen gradient-premium texture-noise flex flex-col items-center justify-center px-6 relative overflow-hidden">
      {/* Premium decorative elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-0 right-0 h-[50vh] bg-gradient-to-b from-primary/5 to-transparent" />
        <motion.div
          className="absolute top-20 left-10 w-80 h-80 rounded-full bg-primary/10 blur-[100px]"
          animate={{ y: [0, -30, 0], scale: [1, 1.1, 1] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
        />
        <motion.div
          className="absolute bottom-20 right-10 w-96 h-96 rounded-full bg-linea-rose/10 blur-[120px]"
        ...
        >
          <div className="relative">
            <div className="w-24 h-24 rounded-3xl gradient-primary shadow-elevated flex items-center justify-center mb-5 mx-auto relative">
              ...
              <svg
                className="w-12 h-12 text-primary-foreground relative z-10"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
      ...
      {/* Bottom tagline */}
      <motion.p
        className="absolute bottom-8 text-sm text-muted-foreground/70 text-center font-medium"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
